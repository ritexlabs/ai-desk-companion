...
class GoogleTool(BaseTool):
    """Unified Google tool: Calendar + Gmail via REST APIs."""
    ...
    async def call_tool(self, tool_name: str, arguments: dict) -> Any:
        ...
        if not token:
            return (
                ...
                'Add GOOGLE_ACCESS_TOKEN to the gateway .env file, '
                'or run the gateway OAuth setup: GET /auth/google'
            )
        ...
        try:
            ...
            if exc.response.status_code == 401:
                return 'Google access token expired. Update GOOGLE_ACCESS_TOKEN in the gateway .env.'
        ...
        async with httpx.AsyncClient(timeout=15.0) as client:
            ...
            for msg in msgs[:3]:
                ...
                if detail_r.status_code == 200:
                    ...
                    subj = next(
                        (hdr['value'] for hdr in hdrs if hdr.get('name', '').lower() == 'subject'),
                        '(no subject)',
                    )
